...
sidebar_label: Multi-Objective-Multi-Fidelity optimization with MOMF
---
...
<LinkButtons
  githubUrl="https://github.com/pytorch/botorch/blob/v0.17.0/tutorials/Multi_objective_multi_fidelity_BO/Multi_objective_multi_fidelity_BO.ipynb"
  colabUrl="https://colab.research.google.com/github/pytorch/botorch/blob/v0.17.0/tutorials/Multi_objective_multi_fidelity_BO/Multi_objective_multi_fidelity_BO.ipynb"
/>

# Multi-fidelity Multi-Objective optimization

In this tutorial notebook we demonstrate how to perform multi-objective multi-fidelity
optimization in BoTorch using the multi-fidelity Hypervolume Knowledge Gradient
(MF-HVKG) [3] and a method called Multi-Objective Multi-Fidelity (MOMF) [1].

MF-HVKG performs one-step lookahead: it operates under the assumption that we can make
one additional observation, and after receiving that additional observation, we will
select the Pareto set of optimal designs. HVKG seeks to select the design `x` to
...
MOMF is an alternative approach that introduces an additional "fidelity objective" that
...
while taking into account the higher computational costs associated with it.

HVKG is generally more cost efficient [3], since it explicitly targets the goal of MF
...
likely give better sample efficiency and performance [3].

In this tutorial, we will optimize a synthetic function that is a modified
...
fidelity objective is a linear function of fidelity, $ f(s)=s$, where $s$ is the
...
$C(s)=\exp(4.8s)$. The goal of the optimization is to find the Pareto front, which is a
trade-off solution set for Multi-objective problems, at the highest fidelity.
...
[1]
[Irshad, Faran, Stefan Karsch, and Andreas Döpp. "Expected hypervolume improvement for simultaneous multi-objective and multi-fidelity optimization." arXiv preprint arXiv:2112.13901 (2021).](https://arxiv.org/abs/2112.13901)

[2]
[S. Daulton, M. Balandat, and E. Bakshy. Parallel Bayesian Optimization of Multiple Noisy Objectives. NeurIPS, 2021.](https://proceedings.neurips.cc/paper/2021/hash/11704817e347269b7254e744b5e22dac-Abstract.html)

[3]
[S. Daulton, M. Balandat, and E. Bakshy. Hypervolume Knowledge Gradient for Multi-Objective Bayesian Optimization with Partial Information. ICML, 2023.](https://proceedings.mlr.press/v202/daulton23a.html)
...
Setting up the global variable that determine the device to run the optimization. The
optimization is much faster when it runs on GPU.
...
The cost_func function returns an exponential cost from the fidelity. The cost_callable
is a wrapper around it that takes care of the input output shapes. This is provided to
the MF algorithms which inversely weight the expected utility by the cost.
...
We use a multi-output SingleTaskGP to model the problem with a homoskedastic Gaussian
...
inversely proportional to the cost: $p(s)=C(s)^{-1}$. The initialization is given a
budget equivalent to 2 full-fidelity evaluations.
...
This is a helper function that initializes, optimizes the acquisition function MOMF and
returns the new_x and new_obj. The problem is called from within this helper function.

A simple initialization heuristic is used to select the 20 restart initial locations
from a set of 1024 random points. Multi-start optimization of the acquisition function
is performed using LBFGS-B with exact gradients computed via auto-differentiation.
...
`get_current_value` optimizes the current posterior mean at the full fidelity to
determine the hypervolume under the current model.

`optimize_HVKG_and_get_obs` creates the MF-HVKG acquisition function, optimizes it, and
returns the new design and corresponding observation.
...
We run MOMF to optimize the multi-fidelity versions of the Branin-Currin functions. The
optimization loop works in the following sequence.

1. At the start with an initialization equivalent to 2 full fidelity evaluations.
1. The models are used to generate an acquisition function that is optimized to select
   new input parameters
1. The objective function is evaluated at the suggested new_x and returns a new_obj.
1. The models are updated with the new points and then are used again to make the next
   prediction.
...
We evaluate performance after every 5 evaluations (this is to speed things up, since
...
We evaluate performance after every evaluation (there are not as many evaluations since
MOMF queries higher fidelities more frequently).
...
Log inference hypervolume regret, defined as the logarithm of the difference between the
...
for multi-information source multi-objective optimization [3].
